...
# c = y-intercept (point on y-axis that line crosses it), when x = 0
# c = (x2 * y1 - x1 * y2) / (x2 - x1)
class Line:
    ...
    def __hash__(self):
        return hash(self.slope) ^ hash(self.intercept)  # hash(intercept) XOR hash(slope)
# @exclude
...
def get_canonical_form(a, b):
    """
    3/6 is represented as the fraction 1/2
    :param a:
    :param b:
    :return:
    """
