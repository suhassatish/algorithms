...
rbclient_egg_path = os.path.join(os.path.dirname("/Users/ssatish/Projects/Ventana/revenge/python/lib/rbtools.py"), "..", "..", "vendor", "python-2.7", "RBTools-0.6.3-py2.7.egg")

in ruby, test with rake task or like this - 
RAILS_ENV=dev_mock script/console
> Release.generate_review_request "cadmium", "1", "/Users/ssatish/Projects/Ventana/output_data/rbdiffs/cadmium1.diff"

testing on smash2 - 
>  Release.generate_review_request "cadmium", "1", "/home/revenge/ci/ci_release/output_data/rbdiffs/cadmium1.diff"
------
...
#abstractmethod can return default values in partial implementation and still force children to override and implement it

-----------------------------
#install anaconda 3.4.1 for mac OS X, downloaded from continuum analytics website (latest as of May 2017); installs python 3.6 along with it
...
# after conda installs all these dependencies, and creates the virtual environment named matcherize for you, you can activate it
...
jupyter --version  # 4.3.0 gives Error executing command 'notebook' on python 3.4; Jupyter notebook only works with python 2.7 and 3.4+
...
best way to implement custom comparators which works in both python2 and python 3 is to use this mixin template - 
...
sorted() - #this function returns a new sorted list, original list is unmodified (ie, not in-place sort)
x = 'bug'
...
In Python 3, __hash__ will be set to None automatically if you define __eq__(), and the object will become unhashable, so for Python 3 you don’t need to override __hash__() unless it is an immutable object and you want to be able to use it as a key value.
...
#Then edit the new migration python file created under alembic/versions to issue the right commands to db to create table.
...
--------------------------------------------
#in python function taking **kwargs argument, keys that are non-strings are not allowed, like int keys.
...
# although int is of fixed precision, sys.maxint + 1 gets promoted to long which is arbitrary precision
...
pySpark known limitations:
  a) IO thruput - python to spark (1.15 MBps), spark to python (9.82 MBps) (or python extension code)
    this can be improved  to 25 MBps by using pickled data frames in pandas and then pushing it down to spark and appending it to RDD
...
# socket server multithreaded connecting with 2 clients concurrently using threads and computing fibonacci. 
# GIL prioritizes things that want to run on the CPU core
# OS gets priority to short running tasks
# response times for longer tasks will become exponentially slow. The solution is to throw the work out to a Process pool. in python 3, 
from concurrent.futures import ProcessPoolExecutor as Pool
future = pool.submit(fib, n)

#threads solve the problem of blocking - waiting for data, send buffer is full, etc. Blocking prevents concurrency.
def countdown(n):
    while n > 0:
        yield n
        n -= 1
